      ...
      <section key="group-type">
        ...
        <div className="space-y-5">
          <div id="field-group-type">
            ...
            <div className={`grid grid-cols-3 sm:grid-cols-6 gap-2 ${formErrors["group-type"] ? "ring-2 ring-red-200 rounded-lg" : ""}`}>{activeGroupTypes.map((gt) => (<label key={gt.value} className={`py-2.5 px-3 rounded-lg text-sm font-medium transition border text-center cursor-pointer ${form.groupType === gt.value ? "bg-cta text-white border-cta" : "bg-white text-text-secondary border-border hover:border-cta/50"}`}><input type="radio" name="groupType" value={gt.value} checked={form.groupType === gt.value} onChange={(e) => { setForm({ ...form, groupType: e.target.value }); clearError("group-type"); }} className="sr-only" required />{gt.label}</label>))}</div>
    ...
    "task-sections": () => (
      <section key="task-sections" id="field-task-sections">
        ...
        {taskSections.length < 3 && (() => {
          ...
          const cols = enabledTypes.length <= 2 ? "grid-cols-2" : "grid-cols-3";
          return (
